        ...
        banner.addEventListener("mousemove", (e) => {
            ...
            for (const key in images) {
                if (images.hasOwnProperty(key)) {
                    ...
                    const element = images[key];
                    const element_data = data_images[key];
                    let b_new = Math.abs(element_data.b + (x_offset / step)); //这里当移动距离为step时才加1px，要幅度大一点就减小step，图片效果由模糊变清晰再变模糊（就是blur减小变为零后再变大）
                    let l_new = 0 - (x_offset / level);
                    element.children[0].style = 'transform: translate(' + l_new + 'px); filter: blur(' + b_new +
                        'px);';
                }
            }
